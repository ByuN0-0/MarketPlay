...
const renderPriceImages = (price: number) => {
  ...
  for (let i = 0; i < numOfFiveThousand; i++) {
    imageArray.push(
      <Image key={`fivethousand-${i}`} src="/image/5000won.png" alt="5000won" width={1000} />
    );
  }
  ...
  for (let i = 0; i < numOfThousand; i++) {
    imageArray.push(
      <Image key={`thousand-${i}`} src="/image/1000won.png" alt="1000won" width={400} />
    );
  }
  ...
  for (let i = 0; i < numOfFiveHundred; i++) {
    imageArray.push(
      <Image key={`fivehundred-${i}`} src="/image/500won.png" alt="500won" width={200} />
    );
  }
  ...
  for (let i = 0; i < numOfHundred; i++) {
    imageArray.push(
      <Image key={`hundred-${i}`} src="/image/100won.png" alt="100won" width={180} />
    );
  }

  // 10원짜리 이미지 추가
  for (let i = 0; i < numOfTen; i++) {
    imageArray.push(<Image key={`ten-${i}`} src="/image/10won.png" alt="10won" width={160} />);
  }
