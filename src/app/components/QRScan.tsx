...
const QRScan: React.FC = () => {
  ...
  const renderPriceImages = (price: number) => {
    const imageArray = [];
    const numOfFiveThousand = Math.floor(price / 5000);
    const numOfThousand = Math.floor(price / 1000);
    const numOfFiveHundred = Math.floor((price % 1000) / 500);
    const numOfHundred = Math.floor((price % 500) / 100);
    const numOfTen = Math.floor((price % 100) / 10);
